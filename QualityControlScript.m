...
for sub = 1:NumSub
    ...
    for scan = 1:numScan
     ...
     if isempty(vtcList)
        ...
        % Missing PRT files
            % PRTs should all be in the same folder as VTCs
            if isempty(prt)
                % Then grab the PRT file in vtcList(v).folder
                % that is for the right task and run,
                % which are both in filename, e.g.:
                % STS3_S2_AVLocal_Run-1_...
                % so the PRT will contain 'run-1_task-AVSpeech_'
                
                filename = vtc.FilenameOnDisk;
                fnameParts = strsplit(filename,'_');
                if ~strcmp(fnameParts{6},'RestingState')
                runNum = extractAfter(fnameParts{7},'Run-');
                
                    cd(HomeDir);
                prtLookup = buildPRT(); % modular code is nice
                    cd(vtcList(v).folder);
                    
                % Get PRT file based on filename
                [~,B] = ismember(fnameParts{6},prtLookup(:,1));
                if ~(B == 9) % Skip BowtieRetino
                    prtChk = dir(strcat('*run-',runNum,'*',prtLookup{B,2},'*.prt'));
                    if ~isempty(prtChk) % just in case nothing found
                        oldPRT = prt;
                        prt = prtChk.name;
                        editFlag = 1;
                    end
                end
                end % if not Resting State
                clear fnameParts prtLookup B prtChk
            end
        ...
        % Commit changes
            if editFlag == 1
                ...
                % Export updated path to new VTC in data dir
                    ...
                    fmrChanged = fmrChanged + 1;
                end
                
                if exist('oldPRT','var')
                % Export updated PRT name to new VTC in data dir
                    vtc.NameOfLinkedPRT = prt;
                    vtc.NrOfLinkedPRTs = 1;
                    prtChanged = prtChanged + 1;
                end
...
% As written, I think this only checks the current working directory
        ...
        % Account for edits
            ...
            output.subject(sub).VTC(v).oldFMR = oldFMR;
                ...
                if exist('oldPRT','var')
            output.subject(sub).VTC(v).oldPRT = oldPRT;
                end
            end
            clear dummy editFlag
