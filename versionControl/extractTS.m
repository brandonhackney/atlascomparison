function output = extractTS(subjectList,atlasName)
...
for i = 1:length(subjectList)
    ...
    for file = 1:length(mtcList)
        ...
        bv(hem).mtcList{tempCount} = mtcList(file).name;
            ...
            filePath = bv(hem).mtc(tempCount).data.LinkedPRTFile;
...
%             end
        ...
        elseif strcmp(hem,'rh')
            ...
            for m = 1:length(bv(h).mtcList)
                ...
                output(i).task(m).run = bv(h).mtc(m).run;
