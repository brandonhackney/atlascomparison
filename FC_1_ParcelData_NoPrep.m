...
for task = 1:size(subData, 2)
    ...
    try
        ...
        if ~isempty(posInd)
            ...
            % get the scans to be included
            vtcList = getCondVTC({subData(task).mtcList.VTCpath}', taskName);
            if strcmp(taskName, 'Motion-Faces')
                vtcList = getCondVTC({subData(task).mtcList.VTCpath}', 'DynamicFaces');
            elseif strcmp(taskName, 'Objects')
                vtcList = getCondVTC({subData(task).mtcList.VTCpath}', 'ComboLocal');
            end
        
        
            % get the task predictors
            preds = subData(task).pred;
            scanID = preds(:, numConds+1:end);

            if size(scanID, 1) == 1   % the old way scan list may be coded
               temp = unique(scanID);
               for i = 1:length(temp)
                   in = find(scanID == temp(i));
                   tempScanList(:, i) = temp(i);
               end
               scanID = tempScanList;
            end
             numScans = size(scanID, 2);

            % only keep going if I can match each run to a vtc (for GSR)
            if size(vtcList, 1) == numScans
    ...
    %             scanIn = 0;
                for scan = 1:numScans
                    in = find(scanID(:, scan) == 1);
                    tempp = preds(in, 1:numConds);
                ...
                for hemi = 1:2
                    ...
                    fprintf(1, 'Task %i: %s, hemi %i... (outIndex: %i)', task, taskName, hemi, taskID);
